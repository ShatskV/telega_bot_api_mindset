...
inline_btn_tag2 = InlineKeyboardButton(_('tag1, tag2, tag3'), callback_data='tags_list')
...
async def process_callback_rating(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await bot.edit_message_text(chat_id=callback_query.from_user.id, 
                message_id=callback_query.message.message_id, text=_('Rating was turned {}').format(rating))
...
async def process_callback_rating(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.finish()
    await bot.edit_message_text(chat_id=callback_query.from_user.id, 
                message_id=callback_query.message.message_id, text=_('Language was changed to English!', locale=new_lang))
...
async def process_callback_rating(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.finish()
    tag_format = tags_format_dict[tags_format.value]
    # tag_value = tags_format.value
    await bot.edit_message_text(chat_id=callback_query.from_user.id, 
                message_id=callback_query.message.message_id, text=_('Tags format change to {}').format(tag_format))
...
async def process_callback_rating(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.finish()
    await bot.edit_message_text(chat_id=callback_query.from_user.id, 
                message_id=callback_query.message.message_id, text=_('Thank you for rating!'))
...
async def rating_off(message: types.Message, state: FSMContext):
    ...
    if not rating:
        ...
        await message.answer(_('Choose rating setting:'), reply_markup=inline_kb_rating)
        return
    rating = rating.lower()
...
async def tags_format(message: types.Message, state: FSMContext):
    ...
    if not tags_fmt:
        ...
        await message.answer(_('Choose tags format:'), reply_markup=inline_kb_tags)
        return 
    ...
    if tags_fmt in TagFormat:
        ...
        tag_fmt = tags_format_dict[tags_fmt.value]
        await message.answer(_('Tags format change to {}').format(tag_fmt))
    else:
...
async def send_welcome(message: types.Message):
    ...
    text = _('<b>Picpack Bot</b>\n'
            ...
            'Visite ower site: https://app.picpack.io/demo_ru\n'
        ...
        async with state.proxy() as data:
            if data.get('inline'):
                await bot.edit_message_text(chat_id=msg.from_user.id, 
                    ...
                    text=_('No action selected!'), reply_markup=None)
    state.finish()


@dp.message_handler(lambda message: message.text and not message.text.lower().startswith('http'), state='Feedback.leave')
async def update_feedback(message: types.Message, state: FSMContext):
    await get_or_create_user_in_db(message)
    await update_user(message.from_user.id, feedback=message.text)
    state.finish()
